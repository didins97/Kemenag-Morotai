...
@section('content')
    <section
        class="relative bg-gradient-to-b from-emerald-50 via-white to-gray-50 py-8 sm:py-12 border-b border-emerald-100/50">
        <div class="container mx-auto px-4">
            <div class="max-w-3xl">
                <!-- Breadcrumb (Diperkecil margin-nya) -->
                <nav class="flex mb-3" aria-label="Breadcrumb">
                    <ol
                        ...
                        <li>
                            <a href="{{ route('berita.index') }}" class="hover:text-emerald-900 transition-colors">
                                Arsip Berita
                            </a>
                        </li>
                        @if(isset($kategori))
                            <li><i class="fas fa-chevron-right text-[8px] opacity-50"></i></li>
                            <li class="text-emerald-900/40">{{ $kategori->kategori }}</li>
                        @endif
                    </ol>
                </nav>

                <!-- Judul (Ukuran dikecilkan & margin bawah dikurangi) -->
                <h1 class="text-2xl md:text-4xl font-black text-emerald-950 mb-2 tracking-tight">
                    @if(isset($kategori))
                        {{ $kategori->kategori }} <span class="text-emerald-600">Berita</span>
                    @else
                        Berita <span class="text-emerald-600">Kemenag</span>
                    @endif
                </h1>

                <!-- Deskripsi (Ukuran teks disesuaikan agar lebih proporsional) -->
                <p class="text-emerald-800/70 text-sm md:text-base leading-relaxed max-w-2xl">
                    @if(isset($kategori))
                        Berita dan artikel terkait {{ $kategori->kategori }} dari Kantor Kementerian Agama Kabupaten Pulau Morotai.
                    @else
                        Informasi resmi, agenda kegiatan, dan perkembangan terkini dari Kantor Kementerian Agama Kabupaten Pulau Morotai.
                    @endif
                </p>
            </div>
        </div>
    ...
    <section class="py-12 bg-gray-50 min-h-screen">
            ...
            <div class="w-full md:w-auto overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
                <div class="flex items-center space-x-2 min-w-max">
                    <a href="{{ route('berita.index') }}"
                        class="px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 {{ !request('kategori') && !isset($kategori) ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-200' : 'bg-white text-gray-600 hover:bg-emerald-50 border border-gray-100' }}">
                        Semua Berita
                    </a>
                    @foreach ($kategories as $kat)
                        <a href="{{ route('berita.index', ['kategori' => $kat->slug]) }}"
                            class="px-5 py-2.5 rounded-xl text-sm font-bold whitespace-nowrap transition-all duration-300 {{ (request('kategori') == $kat->slug || (isset($kategori) && $kategori->id === $kat->id)) ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-200' : 'bg-white text-gray-600 hover:bg-emerald-50 border border-gray-100' }}">
                            {{ $kat->kategori }}
                        </a>
                    @endforeach
                </div>
        ...
        @else
            <div class="relative overflow-hidden bg-white rounded-[3rem] p-12 text-center border border-gray-100 shadow-sm">
